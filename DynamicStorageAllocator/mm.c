...
void* mm_malloc(size_t size) {
  ...
  // gets the pointer to a free block that fulfils the request size constraint
  ptr_free_block = search_free_list(req_size);
   
  // if the pointer is null (no block found), request more space from heap and get the pointer again
  if (ptr_free_block == NULL) {
     request_more_space(req_size);
     ptr_free_block = search_free_list(req_size);
  }
  
  // get the block size of the free block
  block_size = SIZE(ptr_free_block -> size_and_tags);
  
  // removes the free block from the free list
  remove_free_block(ptr_free_block);
  
  // if the allocated block size is greater than sizs, then split the block
  if (block_size - req_size >= MIN_BLOCK_SIZE) {
	// gets the preceding tag used of the free block
	preceding_block_use_tag = (ptr_free_block -> size_and_tags) & TAG_PRECEDING_USED;

	// sets the tags so that they are marked as in use
	...
	// gets a new block with the excess space after the split
	block_info * new_block = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size);
	// sets the tags of new block
	new_block -> size_and_tags = (block_size - req_size) | TAG_PRECEDING_USED;
	// sets the footer of the new block
	*((size_t*) UNSCALED_POINTER_ADD(new_block, block_size - req_size - WORD_SIZE)) = new_block -> size_and_tags;
	// inserts free block into the free lsit
	insert_free_block(new_block);
	
  } else {
	// sets the tag of the free block
	ptr_free_block -> size_and_tags = (ptr_free_block -> size_and_tags) | TAG_USED;
	// gets the next block after the freed block
	block_info* next = ((block_info*)(UNSCALED_POINTER_ADD(ptr_free_block, block_size)));
	// sets the tag of the next block
	next -> size_and_tags = (next -> size_and_tags) | TAG_PRECEDING_USED;
